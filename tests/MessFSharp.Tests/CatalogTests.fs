...
<ruleset name="custom">
    <rule ref="BooleanGetMethodName">
        ...
        try
            ...
            let selection = loadedRuleset.Selections |> List.find (fun s -> s.Name = "BooleanGetMethodName")
            let matchingValue =
                selection.Properties
                |> Seq.tryPick (fun item ->
                    if System.String.Equals(item.Key, "checkParameterizedMethods", System.StringComparison.OrdinalIgnoreCase) then
                        Some item.Value
                    else
                        None)
